...
function copyFile (originFile, newFile) {
  ...
  const isImage = extension === '.jpg' ||
    ...
    extension === '.gif' ||
    extension === '.ico'
  const replaceWithContext = (isImage) ? '' : _utils.transformContextIntoSedString(this.answers)

  return shell(`cat ${originFile} ${replaceWithContext} > ${newFile}`)
