module.exports = function (_dirname) {
  ...
  const _utils = require('../utils')(_dirname)
  ...
  const installDeps = _utils.installDeps
  const logger = _utils.logger
  const sanitize = _utils.sanitize
  const parseWorkingDir = _utils.parseWorkingDir
  const transformContextIntoSedString = _utils.transformContextIntoSedString
  ...
  function copyFile(filename, context) {
    ...
    const isImage = extension === '.jpg' || extension === '.png' || extension === '.jpeg' || extension === '.gif'
    const replaceWithContext = (isImage)? '' : _utils.transformContextIntoSedString(context)

    return Shell(`cat ${SCRIPTS_DIR}${_path}${fname} ${replaceWithContext} > ${CURRENT_DIR}${_dist}${fname}`)
