...
const logger = {
  ...
  error: (err, code, moreinfo) => {
    ...
    if (isVerboseEnabled) {
      ...
      console.error(
        chalk.dim(
          'export YAGG_DEBUG=verbose and re-run last command for non-friendly info'
        )
      )
    }
    console.error(
      chalk.dim(`----------- internal exit code ${code || 1} ----------`)
    )
    console.error()
    process.exit(code)
  },
...
exports.transformContextIntoSedString = function transformContextIntoSedString (
  context
) {
...
exports.getNpmPaths = function getNpmPaths () {
  let paths = []
  ...
  // export YAGG_GENERATORS=~/.nvm/versions/node/$(node -v)/lib/node_modules
  if (process.env.YAGG_GENERATORS) {
    paths.push(process.env.YAGG_GENERATORS)
  }
...
exports.findGeneratorsIn = function findGeneratorsIn (searchPaths) {
  let modules = []
