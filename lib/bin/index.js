...
program
  ...
  .option('-c, --choose', 'choose a template from the list to start your project')
  .action(function listCmd ({ verbose: isVerbose, choose: wantsToChooseTemplate }) {
    ...
    getGlobalDependencies().then(deps => {
      ...
      const templates = deps.map(dep => {
        ...
        const template = (isVerbose) ? ` - ${dep.id}: ${dep.version}` : ` - ${name}`
